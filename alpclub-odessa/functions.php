...
final class Alpclub_Odessa_Theme {
	...
	private function __construct() {
		...
		add_action( 'after_setup_theme', function () {
			// поддержка языков
			load_theme_textdomain( 'surya-chandra-lite', STYLESHEETPATH . '/languages' );
			load_child_theme_textdomain( 'alpclub-odessa', STYLESHEETPATH . '/languages' );
		} );
...
(function(){ var widget_id = 'Is69LzuXqF';var d=document;var w=window;function l(){var s = document.createElement('script'); s.type = 'text/javascript'; s.async = true;s.src = '//code.jivosite.com/script/widget/'+widget_id; var ss = document.getElementsByTagName('script')[0]; ss.parentNode.insertBefore(s, ss);}if(d.readyState=='complete'){l();}else{if(w.attachEvent){w.attachEvent('onload',l);}else{w.addEventListener('load',l,false);}}})();
</script>
...
function aco_get_terms_cpt_by_tax(string $cpt_name, string $taxonomy_name) : array {
	$result = [];
	$args = ['taxonomy' => $taxonomy_name, 'hide_empty' => true];
	...
	foreach($custom_terms as $custom_term) {
		...
		if($query->have_posts()) {
			$result[] = [$custom_term->slug, $custom_term->name];
		}
	}
	return $result;
}
...
function aco_create_cpt_tax_menu_items(string $menu_item_caption, string $post_type, string $taxonomy_name) : string {
	$terms = aco_get_terms_cpt_by_tax($post_type, $taxonomy_name);
	if ( empty($terms) ) {
		return '';
	}

	$s = '<li class="menu-item menu-item-type-custom menu-item-object-custom menu-item-has-children"><a>' . esc_html($menu_item_caption) . '</a>';
	$s .= '<ul class="sub-menu">';
	foreach( $terms as $term ) {
		$s .= '<li class="menu-item menu-item-type-taxonomy">';
		$s .= '<a href="' . esc_attr(get_site_url() . '/' . $taxonomy_name . '/' . $term[0] . '/') . '">' . esc_html($term[1]) . '</a>';
		$s .= '</li>';
	}
	$s .= '</ul>';
	$s .= '</li>';
...
add_filter( 'wp_nav_menu_items', function ($items) {
	$sub_menu_items = aco_create_cpt_tax_menu_items('Направления', AT_TRIP_POST_TYPE, 'destination');
	//$sub_menu_items .= aco_create_cpt_tax_menu_items('Активности', AT_TRIP_POST_TYPE, 'activity');

	if ( !empty($sub_menu_items) ) {
		$find = 'О нас</a></li>';
		$items = str_replace($find, $find . $sub_menu_items, $items);
	}	
